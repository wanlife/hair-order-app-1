...
if uploaded_file:
    ...
    if wb_processed:
        ...
        if len(raw_data) > 0:
            ...
            # 去除重复列名，防止 pyarrow 报错
            unique_headers = make_unique_columns(raw_headers)
            
            df_processed = pd.DataFrame(raw_data[1:], columns=unique_headers)
            
            st.success("✅ 数据解析与处理完成！")
            ...
            with c2:
                # 如果存在尺寸/SKU列，提供筛选
                sku_col = [c for c in df_processed.columns if "尺寸" in c or "SKU" in c or "Size" in c]
                if sku_col:
                    all_skus = [str(x) for x in df_processed[sku_col[0]].dropna().unique() if str(x) != "None"]
            ...
            st.write(f"📊 当前筛选呈现 **{len(df_display)}** 条记录（全表共 {len(df_processed)} 条）:")
            st.dataframe(df_display, use_container_width=True, height=350)

            st.markdown("---")
            
            # 4. 自定义选择输出列
            st.header("4. 🎯 自定义导出列配置")
            ...
            if not selected_cols:
                ...
                # 导出处理
                df_export = df_display[selected_cols]
                
                # 转换回 Excel 文件供下载
                output_buffer = io.BytesIO()
                with pd.ExcelWriter(output_buffer, engine='openpyxl') as writer:
                    df_export.to_excel(writer, index=False, sheet_name="处理后的订单数据")
                ...
                st.download_button(
                    ...
                    file_name=f"处理导出_{uploaded_file.name}",
